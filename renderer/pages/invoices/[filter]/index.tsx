...
export default function InvoicesPage() {
  ...
  const getData = async () => {
    try {
      const fetchedData = await fetch.byDate({ server: 'http://localhost:8000', key: 'hi' });

      switch (router.query.filter) {
        default:
          setData(await fetch.byDate({ server: 'http://localhost:8000', key: 'hi' }));
          break;
      }
  ...
  const getInvoice = async (id) => {
    try {
      ...
      // const json = await ky.get('http://localhost:8000/api/invoice/' + id, {
      //   headers: {
      //     Authorization: "Bearer :Na.0u_WtZ65nhJRybPBA!ara2xhAVCd"
      //   },
      //   // json: { foo: true }
      // }).json<any[]>();
      // invoice = data[id];
            ...
            >
              <div className="h-full px-3 py-4 space-y-4 overflow-y-auto bg-gray-50">
                <ul className="pb-4 space-y-2 font-medium border-b-2 border-b-gray-200">
                  <Link href={'/create?next=' + (data[0]["number"] + 1)}>
                    <Button
                  ...
                  {locations?.map((item, index) => (
                    <li key={index}>
                      <Link
                        ...
                        className={"flex items-center p-2 rounded-lg group hover:bg-gray-200" + (currentPath === "/invoices/location/" + item['id'] + "/" ? "bg-gray-200" : "")}
                      >
